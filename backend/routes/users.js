...
const method = require('./cards'); // переносим сюда кастомный валидатор из route/cards
...
router.patch('/users/me/avatar', celebrate({ // запрос на изменение аватара данного пользователя
  body: Joi.object().keys({
    ...
    avatar: Joi.string().required().custom(method), // добавила везде опцию required чтобы избежать значений null
  }),
}),
updateAvatar);
